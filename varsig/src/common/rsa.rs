...
pub struct Rsa<const HASH: u64> {
    encoding: u64,
    bytes: Vec<u8>,
}

impl<const HASH: u64> Rsa<HASH> {
    pub fn new(encoding: u64, bytes: Vec<u8>) -> Self {
        Self { encoding, bytes }
    }
...
impl<const HASH: u64> VarSigTrait for Rsa<HASH> {
        ...
        R: Read,
    {
        ...
        if hash != HASH {
            return Err(DeserError::Format(RsaError::IncorrectHash(HASH, hash)));
        };
